...
export class DatasetBrowserWidget extends Widget {
  ...
  constructor(
    ...
    base.then(res => {
      ...
      const row = 'input'
      const level = 2
      let row_element = this.node.querySelector(
        "[id='" + btoa(row) + "']"
      ) as HTMLElement

      if (
        row_element.nextElementSibling &&
        atob(row_element.nextElementSibling.id).startsWith(row + '/')
      ) {
        ...
        while (
          ...
          row_element = this.node.querySelector(
            "[id='" + row_element.nextElementSibling.id + "']"
          ) as HTMLElement
          // check if the parent folder is open
          if (
            !open_flag ||
            this.controller[PathExt.dirname(atob(row_element.id))].open
          ) {
...
function activate(
  ...
  console.log(
    'JupyterLab extension jupyterlab-openbayes-dataset-browser is activated!'
  )
...
export function constructDatasetBrowserWidget(
  app: JupyterFrontEnd,
  basepath: string = '',
  ...
  const widget = new DatasetBrowserWidget(
    app,
    basepath,
    id || 'jupyterlab-datasetbrowser'
  )
  ...
  app.commands.addCommand(CommandIDs.toggle + ':' + widget.id, {
    execute: args => {
      ...
      let row_element = widget.node.querySelector(
        "[id='" + btoa(row) + "']"
      ) as HTMLElement
      ...
      if (
        ...
        while (
          ...
          if (
            !open_flag ||
            widget.controller[PathExt.dirname(atob(row_element.id))].open
          ) {
            row_element.style.display = display
          }
        }
